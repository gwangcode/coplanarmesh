...
$$\mathit{local\_avg\_area} = \frac{\mathit{Area}(\mathit{polyA}) + \mathit{Area}(\mathit{polyB})}{2}$$

$$\mathit{safe\_inter\_eps} = \max(\mathit{min\_area\_eps}, \mathit{local\_avg\_area} \times \text{eps})$$
